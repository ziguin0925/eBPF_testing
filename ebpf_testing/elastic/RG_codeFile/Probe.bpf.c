...
{   
    ...
    if(ppid != 1){
        //pid 1 은 init 프로세스
        long sig_1;
        sig_1 = bpf_send_signal(SIGKILL);
        char kill_msg[] = "chmod kill %d";
        bpf_trace_printk(kill_msg, sizeof(kill_msg), sig_1);
        return 0;
    }
    char notkill_msg[] = "chmode not kill";
    bpf_trace_printk(notkill_msg, sizeof(notkill_msg),0);
