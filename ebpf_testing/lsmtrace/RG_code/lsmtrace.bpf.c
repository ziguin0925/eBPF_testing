	 ...
	 struct inode *new_inode_dir, struct dentry *new_dentry)
{
	...
	// get_path_str_from_path(&file_path, path, string_buf);//path ->r_path
	// bpf_printk("--> %s\n", file_path);//get_cmd path
	// u32 uid = bpf_get_current_uid_gid() & 0xFFFFFFFF;
	// bpf_printk(%d , &uid);
	
	// kk 라는 파일 mv 불가(가능) -> file_open에서 경로 설정 가능 확인
	char fname[15];
	bpf_probe_read_str(&fname, sizeof(fname),old_dentry->d_name.name);
	if(bpf_strncmp(&fname,2,"kk") == 0){ 
		bpf_printk("\"kk\" file cannot be moved. %s " , &fname);
		return -EPERM;
	}
	// __u64 pid_tgid = bpf_get_current_pid_tgid();
	// char com_mv= bpf_map_lookup_elem(&permission_denied_map, &pid_tgid);
	 
	// if(com_mv){
	// 	bpf_printk("mv be not allowed in rename %s \n", &com_mv);
	// 	return -EPERM ;
	// }
	...
	bpf_printk("lsm_hook: inode: inode_rename %s",&fname);
	return 0;
}
...
int BPF_PROG(file_open, struct file *file)
{
	...
	// return 0;
	// ls 시에는 해당 
	// char * dname  = BPF_CORE_READ(path, dentry, d_parent, d_name.name); // p_dname
	// char * p_dname[15];
	// bpf_probe_read_str(p_dname, sizeof(p_dname), dname);

	struct com_name command = {};
	bpf_get_current_comm(&command.comm, sizeof(command.comm)); //bpf_printk() 에 출력 or 해당 커맨드 비교
	...
	int ret = get_path_str_from_path(&file_path, path, string_buf);//path ->r_path

	//get_path_str에 넣기
	// dentry 이용 경로 비교 가능 확인
	// get_path_str_from_path에서 file_open 지정 경로에 진입하면 반환 후 command "mv" 인것만 맵(key pid or tgid)으로 넘겨주기.
	// rename 에서 map으로 넘겨준 값 받아 확인 후 return EPERM. 그 다음 map elem delete
	if ( ret == 999 ){
		// if(bpf_strncmp(&command.comm,2,"mv") == 0){
		// 	bpf_map_update_elem(&permission_denied_map, &pid_tgid, &command, BPF_ANY);// 맵 사용 불가 Invalid argument -22 error.h
		// 	bpf_printk("--> %s this  <mv> /home/jlwo0925/go/  EPERM", file_path);
		// 	return -EPERM;
		// }
		// bpf_printk("here is 2506753");
	}
	bpf_printk("--> %s this " , file_path);


	int mode_num; // inode 이용 가능
	mode_num =BPF_CORE_READ(file, f_path.dentry, d_inode, i_mode);
	if (mode_num== 33279){
		bpf_printk("lsm : file_open_mode= 33279");
